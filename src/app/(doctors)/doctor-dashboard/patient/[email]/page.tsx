...
const handleExportPDF = () => {
  ...
  pdf.setFontSize(18);
  pdf.text("MindWell Medical Report", 10, y);
  ...
  pdf.text(`Emotional Score: ${latest?.score || 0}`, 10, y);
  y += 6;

  pdf.text(`Sleep Avg: ${avgSleep} hrs`, 10, y);
  y += 6;

  pdf.text(`Memory Level: ${latestMemory?.level || 0}`, 10, y);
  y += 10;
  ...
  pdf.text(`Thoughts: ${latestSubjective?.thoughts || "None"}`, 10, y, {
    maxWidth: 180,
  });
  ...
  pdf.text(`Memory: ${domainScores.memory}`, 10, y);
  y += 6;

  pdf.text(`Attention: ${domainScores.attention}`, 10, y);
  y += 6;

  pdf.text(`Speed: ${domainScores.speed}`, 10, y);
  y += 6;

  pdf.text(`Executive: ${domainScores.executive}`, 10, y);
  ...
  pdf.text("Generated by MindWell System", 10, 285);
  ...
  return (
    <div ref={reportRef} className="pdf-safe">
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <h2 className="font-bold text-2xl pt-4">Cognitive Analysis</h2>
